...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (body?.mode === 'chunk') {
      ...
      const chars = sections.reduce((sum, s) => sum + (s.text?.length ?? 0), 0);
      if (!sections.length) return res.status(400).json({ error: 'Keine Abschnitte übergeben.' });
      if (chars > MAX_CHUNK_CHARS) return res.status(413).json({ error: 'Analyse-Chunk ist zu groß.' });
    ...
    if (body?.mode === 'finalize') {
      ...
      if (!Array.isArray(partials) || !partials.length) return res.status(400).json({ error: 'Keine Teilergebnisse übergeben.' });
      if (size > MAX_FINAL_PAYLOAD_CHARS) return res.status(413).json({ error: 'Zu viele Teilergebnisse für die Zusammenführung.' });
